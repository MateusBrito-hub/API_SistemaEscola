...
export async function up(knex: Knex){
    return knex
        .schema
        .createTable(ETableName.responsible, table => {
        ...
        .then(() => {
            console.log(`# Create table ${ETableName.responsible}`);
        });
}
...
export async function down(knex: Knex){
    return knex
        .schema
        .dropTable(ETableName.responsible)
        .then(() => {
            console.log(`# Drop table ${ETableName.responsible}`);
        });
}
